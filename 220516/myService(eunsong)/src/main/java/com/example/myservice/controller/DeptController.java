...
//    스프링에 객체로 등록 : 클래스 위에 @Component, @Service, @repository
    ...
    // @Service, @Component, @Bean, @Repository 등등 스프링에 객체를 생성하는 어노테이션
    // @Autowired 스프링에 생성된 객체를 전달받아 아래 멤버변수에 할당함
    @Autowired
    MemberServiceImpl memberService; // 회원서비스 정의

    @Autowired
    BoardServiceImpl boardService; // 게시판 정의
...
//        데이터 추가되었는지 결과 확인( DB Select )
    ...
    // @GetMapping : 아래 URL 메뉴를 Get방식을 이용해 만듬
    // URL Get방식 : select 쿼리에 해당하는 방법(검색, 조회)
...
//        데이터 추가되었는지 결과 확인( DB Select )
    ...
    // 웹브라우저 URL 매개변수{dno}를 메소드 updateMember의 매개변수dno로 연결
    // 웹 애플리케이션 개발 : 웹 URL 형태
    // 형태 1(전통) : http://127.0.0.1/read?no=1 (파라미터 형태)
    // @GetMapping("read")
    // 메소드의 매개변수 : @RequestParam("no") int no
    // 형태 2(REST API) : http://127.0.0.1/read/1 (Rest API, axios 이용)
    // @GetMapping("read/{no}")
    // 메소드의 매개변수 : @PathVariable("no"-생략가능) int no
    @PutMapping("/dept/{dno}")
    public List<Dept> updateMember(@PathVariable int dno, @RequestBody Dept dept) {

        // update 문 실행
        deptService.updateMember(dno, dept);
        // 데이터 수정되었는지 전체 데이터 다시 조회회
       return deptService.selectAll();
    }
