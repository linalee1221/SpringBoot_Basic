...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      UploadService.upload(this.currentImage, (event) => {
        // 파일이 업로드 될 때 진척상황이 저장됨
        this.progress = Math.round((100 * event.loaded) / event.total);
      })
        // 성공하면 서버에서 응답한 객체가 then으로 들어옴
        .then((response) => {
          // 서버 쪽 응답 메시지 저장
          this.message = response.data.message;
          // 서버 쪽에 insert가 잘 되었는지 select 문 호출
  ...
  // 화면이 뜨자마자 실행되는 이벤트
  mounted() {
    // springboot 서버에 이미지 파일 요청
    UploadService.getFiles()
      // 성공하면 then에 객체와 응답메세지가 들어옴
      .then((response) => {
        // response.data : 서버 쪽에서 전송된 객체
